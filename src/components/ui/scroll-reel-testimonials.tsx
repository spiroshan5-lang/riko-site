...
const QUOTE_CLASSES =
  "m-0 text-lg font-medium leading-[1.3] tracking-[-0.02em] text-sand sm:text-[22px]";
const AUTHOR_CLASSES = "m-0 text-sm font-medium leading-[1.3] text-muted-foreground";
...
export function ScrollReelTestimonials({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        "relative flex w-full max-w-[1060px] flex-col items-stretch gap-2.5 overflow-hidden rounded-xl border border-accent/25 bg-gradient-to-br from-cocoa/90 via-cocoa/75 to-terracotta/25 shadow-[0_8px_32px_rgba(0,0,0,0.5),inset_0_1px_1px_rgba(255,255,255,0.08)] backdrop-blur-md outline-none focus-visible:ring-2 focus-visible:ring-ring md:min-h-[320px] md:flex-row",
        className,
      )}
    >
      {/* Reel section */}
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-5 py-7 md:py-10">
        <div className="flex flex-col gap-[9px]">
          <svg
            className="block h-12 w-12 text-accent/50"
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          >
        ...
        <div className="mt-6 flex items-center gap-1.5 md:mt-0">
          <button
            ...
            className="grid h-6 w-6 cursor-pointer place-items-center rounded-full border border-sand/30 bg-transparent p-0 text-sand transition-[opacity,transform] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] hover:enabled:scale-[1.08] hover:enabled:border-accent active:enabled:scale-[0.94] disabled:cursor-default disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          >
            <svg
